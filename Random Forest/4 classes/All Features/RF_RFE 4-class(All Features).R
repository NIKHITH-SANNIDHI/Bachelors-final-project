...
#This program implements multiclass Random Forest-RFE
...
final_list <-RF_RFE(BIG.A[1:45], BIG.A$TARGETtype)
...
write_xlsx(Feature_names, path = "D:\\ENGINEERING\\SEMESTER-8\\Project Work\\Experiment\\Random Forests\\4 class\\All Features\\RankList using RF_RFE-4 classes(All Features).xlsx",
           col_names = FALSE)
library(readxl)

there<-read_excel("RankList using RF_RFE-4 classes(All Features).xlsx",
           col_names = FALSE)
length(there)
