...
SVM_RFE <- function(x,y){
      ...
      while(length(survivingFeaturesIndices)>0)
      {
            ...
            best_GammaValues<-sigest(y~., data = x) #By default, quater of data is used
...
final_list <-SVM_RFE(BIG.A[1:45], BIG.A$TARGETtype)
...
write_xlsx(Feature_names, path = "D:\\ENGINEERING\\SEMESTER-8\\Project Work\\Experiment\\SVM_RFE implementation\\4 class\\All Features\\RankList using SVM_RFE-4 classes(All Features).xlsx",
           col_names = FALSE)
